# -*- coding: utf-8 -*-
"""Pre-ggplot copy with streamlit of EEP 105 Final Project.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1TcbOVmuv8yx_DHkpucxQYQiVrYYYvkTn
"""
...
# Scatterplot (the one with two scatter plots on top of each other)
# copied from chloe's individual project
df_co2_c.rename(columns={'Emissions': 'Value'}, inplace=True)
df_co2_c['Indicator'] = 'Emissions'

data_long = pd.concat([df_co2_c, df_temp_c])

data_long = data_long[(data_long['Country'] == 'United States') &
                      (data_long['Year'] >= 1980) &
                      (data_long['Year'] <= 2014)]
...
g.set_xticklabels(range(1980, 2015, 5), rotation=90)
...
g.fig.suptitle("SK Emissions and Temperatures (1980-2014)", y=1.02)
# saving plot as a png file
plt.savefig('my_plot2.png', dpi=300, bbox_inches='tight')

plt.show()

# 5) Recreate the two scatter plots in the “Scatter Plot” subsection, including the trend lines on the figures.

# rename the 'Emissions' column to 'Value' in the CO2 DataFrame to match the Temperature DataFrame's column name.
#add an 'Indicator' column to the CO2 DataFrame to identify the data type
# combine the two dataframes into a single "long" format DataFrame
# Filter the data for the specified country and years
# create the plot
# add scatter plot points and a loess smoothed line
# customize the plot
df_co2_c.rename(columns={'Emissions': 'Value'}, inplace=True)
...
#the result is a wide format dataframe
#select the necessary columns
#customize the plot
#Display the plot

df_co2_c.rename(columns={'Value': 'Emissions'}, inplace=True)
df_temp_c.rename(columns={'Value': 'Temperature'}, inplace=True)


wide_US = pd.merge(df_co2_c, df_temp_c, on=['Year', 'Country'])
...
df_co2_c.rename(columns={'Emissions': 'Value'}, inplace=True)
df_co2_c['Label'] = 'CO2 Emissions (Metric Tons)'

# Prepare the Temperature data
df_temp_c['Label'] = 'Temperature (Celsius)'

# Concatenate the prepared dataframes
data_long = pd.concat([df_co2_c, df_temp_c])

# Filter the data for 'South Korea' and the years 1980-2014
data_long = data_long[(data_long['Country'] == 'South Korea') &
                      (data_long['Year'] >= 1980) &
                      (data_long['Year'] <= 2014)]
...
g.fig.suptitle("South Korea Emissions and Temperatures (1980-2014)", y=1.02)
plt.savefig('south_korea_plot_fixed.png', dpi=300, bbox_inches='tight')
plt.show()
