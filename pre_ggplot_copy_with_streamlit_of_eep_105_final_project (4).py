...
sd_y_su = np.std(list(df_sk_co2_temp_wide_std["Temperature"]))

output_string = (
    f"Emissions mean (su): {mean_x_su:.2f} "
    f"Temperature mean (su): {mean_y_su:.2f} "
    f"Emissions standard deviation (su): {sd_x_su:.2f} "
    f"Temperature standard deviation (su): {sd_y_su:.2f}"
)
...
st.write(
    f"**Correlation coefficient:** {coeff.statistic:.4f}"
)

st.write(
    f"**P-value:** {coeff.pvalue:.4f}"
)

st.markdown(f"### Correlation Results")
st.markdown(f"The Pearson correlation coefficient between Emissions and Temperature is **{coeff.statistic:.4f}**.")
st.markdown(f"The corresponding p-value is **{coeff.pvalue:.4f}**.")
# streamlit ver
